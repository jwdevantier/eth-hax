"use strict"
const Eth = require('ethjs');
const http = require('http');
const eth = new Eth(new Eth.HttpProvider('http://localhost:8545'));

const deposit = (body) => {
    const refId = body['ref-id']
    const amount = body['amount']
    console.log('About to deposit', JSON.stringify(body, null, 2))
}
const withdraw = (body) => {
    // do same as deposit
}

var express = require('express')
  , bodyParser = require('body-parser')
  , cors = require('cors')

var app = express();
app.use(cors())
app.use(bodyParser.json());

app.post('/', function(request, response){
   const body = request.body
   if ( ! body) return

   const typeOfForm = body.typeOfForm
   switch (typeOfForm) {
       case "deposit": return deposit(body)
       case "withdraw": return withdraw(body)
       default: return true;
   }
});

app.listen(8080);


// 
//  ETHJS example code
// 

// eth.getBlockByNumber(3, true, (err, block) => {
//     //console.log(err, block)
//   // result null { ...block data... }
// });


// const accounts = (returnData) => eth.accounts(function(accountsError, accounts) {
//     accounts.forEach(function(account){
//         returnData(account)
//         console.log(account)
//       eth.getBalance(account, function(balanceError, balance){
//         console.log(balance)
//         Eth.fromWei(balance, 'ether')
//       });
//     });
//   });



//   eth.accounts().then(function(accounts) {
//     var SimpleStoreBytecode = '';
//     var SimpleStoreABI = [{"constant":false,"inputs":[{"name":"_value","type":"uint256"}],"name":"set","outputs":[],"payable":false,"type":"function"},{"constant":true,"inputs":[],"name":"get","outputs":[{"name":"","type":"uint256"}],"payable":false,"type":"function"}];
//     var SimpleStore = eth.contract(SimpleStoreABI, SimpleStoreBytecode, { from: accounts[0], gas: 300000 });
//     var simpleStoreInstance;

//     SimpleStore.new({}).then(function(txHash){
//       var checkTransaction = setInterval(function(){
//         eth.getTransactionReceipt(txHash).then(function(receipt){
//           if (receipt) {
//             clearInterval(checkTransaction);
//             simpleStoreInstance = SimpleStore.at(receipt.contractAddress);
//           }
//         });
//       }, 400);
//     })
//     .catch(function(error){
//         console.log(String(error))
//     });
//   });

// const etherValue = Eth.toWei(72, 'ether');

// // result <BN: 3e733628714200000>

// const tokenABI = [{
//   "constant": true,
//   "inputs": [],
//   "name": "totalSupply",
//   "outputs":[{"name": "","type": "uint256"}],
//   "payable": false,
//   "type": "function",
// }];

// const token = eth.contract(tokenABI).at('0x6e0E0e02377Bc1d90E8a7c21f12BA385C2C35f78');

// token.totalSupply().then((totalSupply) => {
//   // result <BN ...>  4500000
// });